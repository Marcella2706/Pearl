...
export const ChatMessage = ({ message, isAI, timestamp, type = "general" }: ChatMessageProps) => {
  ...
  return (
    <div className={cn(
      ...
      {isAI && (
        <div className="shrink-0 w-10 h-10 bg-card/50 backdrop-blur-sm border border-border rounded-full flex items-center justify-center">
          {getIcon()}
        </div>
      )}
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          isAI 
            ? "rounded-tl-md bg-card border-border" 
